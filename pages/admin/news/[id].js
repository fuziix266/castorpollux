  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Error al actualizar la noticia: ' + (error.error || 'Error desconocido'))
      }
    } catch (error) {
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            />
            {formData.image_url && (
              <div className="flex items-center gap-4 p-4 border border-slate-300 dark:border-slate-700 rounded-lg mt-2">
                <div
                  className="bg-center bg-no-repeat aspect-video bg-cover rounded-lg w-32 h-20"
